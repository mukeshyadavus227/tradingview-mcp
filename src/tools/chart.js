...
export function registerChartTools(server) {
  ...
  }, ({ timeframe }) => wrapCall(() => core.setTimeframe({ timeframe })));
  ...
  }, ({ chart_type }) => wrapCall(() => core.setType({ chart_type })));
  ...
  }, ({ action, indicator, entity_id, inputs }) => wrapCall(() => core.manageIndicator({ action, indicator, entity_id, inputs })));

  server.tool('chart_get_visible_range', 'Get the visible date range (unix timestamps) and bars range on the chart', {},
    () => wrapCall(() => core.getVisibleRange()));
  ...
  }, ({ from, to }) => wrapCall(() => core.setVisibleRange({ from, to })));
  ...
  }, ({ query, type }) => wrapCall(() => core.symbolSearch({ query, type })));
}
